...
<header class="logHead">
    <span class="inteTit" style="vertical-align: middle;color: #fff;font-size: .54rem;">积分:</span>
    <span class="inteNum" style="vertical-align: middle;color: #fff;">{{$user['score']}}</span>
</header>
<ul class="logList" id="mescroll">
    {{--@if($allLog->count())
        @foreach($allLog as $log)
            <li>
                <span>{{$log->text}}</span>
                <span @if($log->num > 0) style="color: green" @else style="color: red"@endif>@if($log->num > 0)+@endif{{$log->num}}</span>
                <span style="font-size: .3rem;float: right;display: block;">{{$log->c_time}}</span>

            </li>
        @endforeach
    @else
        <span style="text-align: center;display: block;padding: 1rem;font-size: .45rem;">暂无记录,赶紧动起来吧!</span>
    @endif--}}
...
<script>
    ...
    function PJHTMLren(item) {
        let HTMLBank = `<li>
                <span>${item.text}</span>
                <span >${item.num > 0 ? '+'+item.num : item.num}</span>
                <span style="float: right;display: block;color: #adadad;">${item.c_time}</span>
