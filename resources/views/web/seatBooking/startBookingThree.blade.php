...
<div class="">
    <img id="imgID" src="{{$storage->url($region->img)}}" style="width: 100%" alt="">
</div>
...
<script>
    ...
    const configNum="{{$globalConfig->num}}";
    const meNum="0";
    const backUrl="{!! route('Seat::startBookingTwo',['token'=>request()->input('token'), 'id'=>request()->input('id')]) !!}";
    const csrf_token = "{{csrf_token()}}";
    var goOK=$('#goOK');

    function initSeat(area_id) {
        if (meNum>=configNum){
            popu('抱歉，你预约的座位数已达上限，不能再进行预约了');
            return false;
        }
    ...
    function initChart(data) {
        ...
        $.each(data.list,function(index,value){
            var selected = (value.state=="1" ? 'selected' : ''); //已经预约
        ...
        $('.seats li input').on('click', function () {
            ...
            if (parseInt(meNum)+checklen > configNum ) {
                var oknum=parseInt(checklen)-1;
                popu('您目前只能预约 '+oknum+' 个位置了。');
                return false;
            }
...
<script type="text/javascript">
    ...
    $(document).on('tap', '.nextUrl', function(){
        ...
        checkArr.each(function (e) {
            data.push([$(this).data('id'),$(this).data('num')]);
            str+=$(this).data('num');
            if (checkArr.length-1 != e){
                str+='、';
            }
        })
        console.log(id);
        ...
        console.log(str);
        layer.open({
            content: '您确定要预约 '+str+' 号位置 ?'
            ,btn: ['是的', '取消']
            ,yes: function(index){
                layer.open({
                    type: 2
                    ,content: '预约中...',shadeClose:false
                });
                $.ajax({
                    type: "post",
                    url: "{{route('Seat::submitBooking',['token'=>request()->input('token')])}}",
                    data: {"data": data,"id":id,"etime":etime,"stime":stime},
                    dataType: "json",
                    headers: {
                        'X-CSRF-TOKEN': csrf_token
                    },
