...
class NewIndexController extends BaseController
{
    ...
    public function __construct(Request $request)
    {
        ...
        View::share('groupList', VoteGroup::getCache($this->token, $this->aid));
    }
    ...
    public function ajaxItems(Request $request)
    {
        ...
        if ($searchKey) {
            ...
            $items = VoteItems::select('id', 'cover', 'number', 'title', 'info', 'created_at')
            ...
            $items = Cache::remember($cacheKey, 30, function () use ($where) {
                return VoteItems::with([
                    'fans' => function ($query) {
                        $query->where('token', $this->config['token']);
                    }
                ])
                    ->select('id', 'cover', 'number', 'title', 'info', 'created_at', 'openid')
                    ->where($where)
                    ->orderBy('number')
                    ->paginate(10);
            });
        ...
        foreach ($items as $k => $v) {
            $items[$k]->view_n = Arr::get($viewlist, $v->id, 0);
            $items[$k]->voting_n = Arr::get($list, $v->id, 0);
            $items[$k]->url = route('Vote::details2', Arr::add($this->basisWhere, 't_id', $v->id));
        }
        return $items;
    }
    ...
    public function ajaxComment(Request $request)
    {
        ...
        $items = VoteMessage::with('fans')
            ->select('id', 'content', 'create_at', 'f_id')
            ->where($where)
            ->paginate(10);
        return $items;
    }
    ...
    public function details(Request $request, WebOAuthService $webOAuthService)
    {
        ...
        $qrCode = Wxuser::getCache($this->token)->qr_code;
        $t_id = $request->input('t_id');
        $details = VoteItems::with('fans')->where(['a_id' => $this->aid, 'id' => $t_id])->first();
        $fields = VoteField::where('g_id', $details['g_id'])->get()->keyBy('id');
        ...
        return \view('web.newVote.details', [
            'fansInfo' => $fansInfo,
            'fields' => $fields,
            'details' => $details,
            'groupInfo' => $groupInfo,
            ...
            'urlArr' => $this->initUrl2($details['g_id']),
        ]);
    }
    ...
    public function rankList(Request $request, WebOAuthService $webOAuthService)
    {
        ...
        foreach ($voteRank as $k => $v) {
            ...
            $rankList[] = [
                ...
                'votes' => $v['votes'],
                'url' => route('Vote::details2', Arr::add($this->basisWhere, 't_id', $first['id'])),
            ];
        }
        return \view('web.newVote.rank', [
            'sliderImg' => $this->getSliderImg(),
            'fansInfo' => $fansInfo,
            'myItemID' => $myItemID,
    ...
    public function signUp(Request $request, WebOAuthService $webOAuthService)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            if ($status) {
                ...
                return redirect(route('Vote::details2', Arr::add($this->basisWhere, 't_id', $id)));
            }
            return back()->withErrors('系统繁忙请稍后再试!');
        }
        return \view('web.newVote.signup', [
            'fansInfo' => $fansInfo,
            'myItemID' => $myItemID,
            'groupInfo' => $groupInfo,
            'fields' => $fields,
     ...
     * @return \Illuminate\Contracts\View\Factory|\Illuminate\View\View
     */
    public function explain_bak(Request $request, WebOAuthService $webOAuthService)
    {
        $fansInfo = $webOAuthService->checkOauth();
        $g_id = $request->input('g_id');
        $groupList = VoteGroup::getCache($this->token, $this->aid);
        ...
        //查看是否已报名
        $myItemID = VoteItems::findOpenid($this->aid, $g_id, $fansInfo['openid'])->value('id');
        return \view('web.newVote.explain', [
            'sliderImg' => $this->getSliderImg(),
            'fansInfo' => $fansInfo,
            'myItemID' => $myItemID,
            'groupData' => $this->getGroupData($g_id, $groupList),
    ...
    public function explain(Request $request, WebOAuthService $webOAuthService)
    {
        ...
        //查看是否已报名
        $myItemID = VoteItems::findOpenid($this->aid, $g_id, $fansInfo['openid'])->value('id');
        return \view('web.newVote.explain2', [
            'sliderImg' => $this->getSliderImg(),
            'fansInfo' => $fansInfo,
            'myItemID' => $myItemID,
            'groupData' => $this->getGroupData($g_id, $groupList),
     ...
     * @return mixed
     */
    public function comment(Request $request, WebOAuthService $webOAuthService)
    {
        $g_id = $request->input('g_id');
        $t_id = $request->input('t_id');
        ...
        if ($this->config['rules_ip'] == 1) {
            ...
            $exists = VoteBlacklist::where(['token' => $this->token, 'ip' => $checkIp])->exists();
            if ($exists) {
                return $this->message('抱歉,黑名单用户无法进行留言!', false);
            }
            $exists = VoteBlacklist::where(['token' => $this->token, 'openid' => $fansInfo['openid']])->exists();
            if ($exists) {
                return $this->message('抱歉,黑名单用户无法进行留言!', false);
        ...
        $fans = Fans::where(['token' => $this->config['token'], 'openid' => $fansInfo['openid']])->first();
        $create = [
            'a_id' => $this->aid,
            'g_id' => $g_id,
            't_id' => $t_id,
            ...
            'openid' => $fansInfo['openid'],
            'content' => $content,
            'status' => $this->config['comment_sw'] == 1 ? 1 : 0
        ];

        $status = VoteMessage::create($create);
        if ($status) {
//            $id = $status['id'];
//            $this->addRedisList($g_id, 'view', $id);
            return $this->message('留言成功,等待刷新!', true);
        }
        return $this->message('留言失败,请稍后再试!', true);
    }
}
