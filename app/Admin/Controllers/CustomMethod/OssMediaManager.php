...
namespace App\Admin\Controllers\CustomMethod;
 ...
 * Class MediaManager.
 */
class OssMediaManager extends Extension
{
...
//        dd($directories);
    ...
    public function download()
    {
        ...
        if ($this->storage->exists($fullPath)) {
            ...
            return response()->streamDownload(function () use ($fullPath) {
                echo file_get_contents($this->storage->url($fullPath));
            }, $fileName);
    ...
    public function delete($path)
    {
        ...
        foreach ($paths as $path) {
            ...
            $pathName = pathinfo($fullPath, PATHINFO_BASENAME);
            $is_file = str_contains($pathName, '.');
            if ($is_file) {
    ...
    public function urls()
    {
        return [
            'path' => $this->path,
            'index' => route('ossMedia-index'),
            'move' => route('ossMedia-move'),
            'delete' => route('ossMedia-delete'),
            'upload' => route('ossMedia-upload'),
            'new-folder' => route('ossMedia-new-folder'),
        ];
    }
    ...
    public function formatFiles($files = [])
    {
        $files = array_map(function ($file) {
            return [
                'download' => route('ossMedia-download', compact('file')),
                'icon' => '',
                'name' => $file,
                'preview' => $this->getFilePreview($file),
                'isDir' => false,
//                'size' => $this->getFilesize($file),
                'link' => route('ossMedia-download', compact('file')),
...
//                'time' => $this->getFileChangeTime($file),
    ...
    {
        $url = route('ossMedia-index', ['path' => '__path__', 'view' => request('view')]);
        ...
        $dirs = array_map(function ($dir) use ($preview) {
            ...
            return [
                ...
                'isDir' => true,
                'size' => '',
                'link' => route('ossMedia-index', ['path' => '/' . trim($dir, '/'), 'view' => request('view')]),
                'url' => $this->storage->url($dir),
                'time' => $this->getFileChangeTime($dir, 1),
            ];
        }, $dirs);
    ...
    public function navigation()
    {
        ...
        foreach ($folders as $folder) {
            ...
            $navigation[] = [
                'name' => $folder,
                'url' => route('ossMedia-index', ['path' => $path]),
            ];
        }
    ...
    {
        $bytes = $this->storage->size($this->getFullPath($file));
    ...
    public function getFileChangeTime($file, $isDir = 0)
    {
        $time = ($isDir) ? time() : $this->storage->lastModified($this->getFullPath($file));
