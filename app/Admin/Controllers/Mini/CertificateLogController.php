    ...
    protected function grid()
    {
        ...
        $grid->actions(function ($actions) {
            ...
            if ($actions->row->is_pay == 1) {
                $actions->append("<button class='btn btn-xs btn-success margin-r-5 checkPay' data-oid='{$actions->row->order_id}'
 data-token='{$actions->row->mini_token}'>核对支付</button>");
            }
            if ($refunds < $price && in_array($actions->row->orders['pay_status'], [1, 2])) {
                $re = $price - $refunds;
                $actions->append("<button class='btn btn-xs btn-danger margin-r-5 refundPay' data-oid='{$actions->row->order_id}'
 data-pr='{$price}' data-re='{$re}' data-token='{$actions->row->mini_token}' data-pubtoken='{$actions->row->token}'>退款</button>");
            }
            if ($actions->row->status == 2) {
                $actions->append("<button class='btn btn-xs btn-warning margin-r-5 reapply' data-name='{$actions->row->name}'
data-id='{$actions->row->id}' data-token='{$actions->row->mini_token}'>异常补办</button>");
            }
            /*if ($actions->row->status == 4) {
                $actions->append("<button class='btn btn-xs btn-primary margin-r-5 audit' data-name='{$actions->row->name}'
data-id='{$actions->row->id}' data-token='{$actions->row->mini_token}'>审核</button>");
            }*/
        });
    ...
    public function reapplyReader(Request $request)
    {
        ...
        $certificateLog = CertificateLog::where(['mini_token' => $token, 'id' => $id, 'status' => 2])->first();
        if (!$certificateLog) {
            $re['mes2'] = '非法操作,参数错误!';
            return $re;
        }
        ...
        if ($certificateLog['is_pay'] == 1 && $certificateLog['order_id']) {
            $order = CertificateOrders::where(['mini_token' => $token, 'order_id' => $certificateLog['order_id']])
                ->first(['price', 'transaction_id']);
            if ($order) {
                $newReader['deposit'] = $order['price'];
                $newReader['serialno'] = $order['transaction_id'];
    ...
    {
        $checkPayUrl = route('mini.certificate.checkPay');
        $refundPayUrl = route('mini.certificate.refundPay');
        $reapplyReaderUrl = route('mini.certificate.reapplyReader');
    ...
    swal({
         ...
         return new Promise(function(resolve, reject) {
             $.ajax({
                        url: "{$checkPayUrl}",
                        type: "post",
                        data: {"_token": LA.token,"order_id":order_id,"token":token},
                        dataType: "json",
...
$('.refundPay').on('click', function () {
    ...
    var token = $(this).data('token');
    var pubtoken = $(this).data('pubtoken');
    swal({
         title: '请输入需要退款金额',
         text: '可退款金额 '+ re_price,
         input: 'number',
