...
class DfArrearsLogController extends Controller
{
    ...
    {
        $grid = new Grid(new DfArrearsOrders());
...
//                    break;
    ...
    {
        $checkPayUrl = route('wechat.dfArrears.checkPay');
        $refundPayUrl = route('wechat.dfArrears.refundPay');
        $reapplyUrl = route('wechat.dfArrears.reapply');
...
SCRIPT;
    ...
    public function refundPay(Request $request, PayHelper $payHelper)
    {
        ...
        $app = WechatPay::initialize($token);
        $refund_order_id = $payHelper->GenerateOrderNumber('DFQK-TK');

        $response = $app->refund->byOutTradeNumber($order_id, $refund_order_id, $price * 100, $refundNumber * 100, [
            'refund_desc' => '代付欠款金额返还',
            'notify_url' => route('WxRefund_dfArrears', $token),
        ]);
        ...
        if (array_get($response, 'return_code') === 'SUCCESS') {
            ...
            if (array_get($response, 'result_code') === 'SUCCESS') {
                ...
                DfArrearsRefund::create($refundLog);
                DfArrearsOrders::where('order_id', $order_id)->update(['pay_status' => 2]);
...
//        //根据id查看状态
        ...
        $arrearsLog = DfArrearsLog::where(['token' => $token, 'id' => $id, 'status' => 2])->first();
        if (!$arrearsLog) {
            $re['mes2'] = '非法操作,参数错误!';
            return $re;
        }
