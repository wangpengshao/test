...
class LuckyDraw02Controller extends Controller
{
    ...
    protected function grid()
    {
        ...
        $grid->is_bind('需要绑定?')->switch();
        $grid->is_subscribe('需要关注?')->switch();
        $grid->count('抽奖次数');
        $grid->start_at('开始时间');
        $grid->end_at('结束时间');
        $grid->status('活动状态')->switch();
        $grid->actions(function ($actions) {
            $url = route('type02List.index', ['l_id' => $actions->row->id]);
            $actions->append(new IconButton($url, '当前活动抽奖列表', 'fa-list'));
            $url = route('luckyDraw.prize', ['lucky_type' => 2, 'l_id' => $actions->row->id]);
    ...
    {
        $show = new Show(LuckyDraw02::findOrFail($id));
        $show->id('抽奖地址')->as(function ($id) {
            return route('LuckyDraw02::home',['token'=>session('wxtoken'),'l_id'=>$id]);
        });
        $show->updated_at('最后编辑时间');
        $show->divider();
        // 抽奖总人数(openid分组)
        $totalNumber = DB::table('wechat_luckydraw_02_list')
        ...
        $totalFrequency = DB::table('wechat_luckydraw_02_list')
            ->where(['l_id'=>$id, 'token'=>session('wxtoken')])
            ->count();
