...
class LuckyDraw03Controller extends Controller
{
    ...
    protected function grid()
    {
        ...
        $grid->actions(function ($actions) {
            ...
            $url = route('luckyDraw.prize', ['lucky_type' => 3, 'l_id' => $actions->row->id]);
    ...
    {
        $show = new Show(LuckyDraw03::findOrFail($id));
        $show->id('抽奖地址')->as(function ($id) {
            return route('LuckyDraw03::home', ['token' => session('wxtoken'), 'l_id' => $id]);
        });
        $show->updated_at('最后编辑时间');
        $show->divider();
        // 抽奖总人数(openid分组)
        $totalNumber = DB::table('wechat_luckydraw_03_list')
        ...
        $totalFrequency = DB::table('wechat_luckydraw_03_list')
            ->where(['l_id'=>$id, 'token'=>session('wxtoken')])
            ->count();
...
//            ->help('用于抽取次数记录计算,微信标识则是微信用户openid做单位计算,如证号单位请设置好绑定读者才能抽奖');
        ...
        $form->number('number', '可砸次数')->max(9)->default(0)
            ->help('如按天算,这里参数就是用户每天可砸次数,如按期数算，则是当前活动用户可砸次数,<span style="color: red">最多可砸:9次</span>');
