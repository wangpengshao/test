...
class TransactCardLv2Controller extends Controller
{
    ...
    public function index()
    {
        return Admin::content(function (Content $content) {
            $content->header('实名办证配置');
            $content->description('编辑');
            $content->row(function (Row $row) {

                $row->column(8, function (Column $column) {
                    $data = Certificate::where(['token' => session('wxtoken'), 'type' => 1])->first();

                    $form = new \Encore\Admin\Widgets\Form($data);
                    $url = ($data) ? route('wechat.certificateLv2.up', $data->id) : route('wechat.certificateLv2.add');
                    $form->action($url);
                    $states = [
                    ...
                    $op = TransactType::where('token', session('wxtoken'))->pluck('title', 'id');
                    if (count($op) == 0) {
