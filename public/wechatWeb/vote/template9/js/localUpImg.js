...
$.fn.localUpImg = function(obj) {
    this.on('change',function () {
        ...
        if(file.type.split("/")[0]!=="image"){
            layer.open({content: '请上传图片',skin: 'msg',time: 3})
        }
        if(file.size>obj.size*1024*1024){
            layer.open({content: '上图上传大小限制'+obj.size+'M',skin: 'msg',time: 3})
        }
    ...
    function _create(blob) {
        ...
        img.onload = function() {
            ...
            var peoplist = document.querySelector('#'+obj.group);

            //var addpic = document.getElementsByClassName('add-pic')[0];
            var addpic = document.querySelectorAll('#'+obj.group+' .add-pic')[0];
            peoplist.insertBefore(NewNode, addpic);

            var mypictureLength = document.querySelectorAll('#'+obj.group+' .mypicture');

            if( mypictureLength.length >= obj.num) {
